...
app.get('/getProductos',(req,res)=>{
	conn.connect().then(function () {
        var request = new sql.Request(conn);
        request.query(" SELECT Production.Product.ProductID, Product.Name FROM production.product order by Production.Product.ProductID asc").then(function(recordset) {
        	res.json(recordset);
        })
        .catch(function (err) {
        	res.send("Not result");
